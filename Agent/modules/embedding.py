...
def index_user_profile(user_id: str) -> int:
    ...
    # Enhanced text extraction - include all profile fields
    text_fields = []
...
def retrieve_chunks(user_id: str, query_text: str, top_k: int, namespace: str = "profile") -> List[Dict[str, Any]]:
    # Autonomous indexing: check if user is indexed, if not, do it.
    ...
    # Always include essential chunks (name, contact info) regardless of semantic similarity
    ...
    # Add a score of 1.0 to essential chunks and convert to same format
    ...
    # Limit to top_k + number of essential chunks (but reasonable limit)
...
def compute_semantic_score(text1: str, text2: str) -> float:
    ...
    return (cosine_score + 1) / 2 # Scale to 0-1 range